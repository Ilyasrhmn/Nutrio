...
export default function DailyScorePage() {
  const scoreData = {
    ...
    checkpoints: [
      ...
      { id: "CP3", label: "Makanan Siap", score: 18, status: "warning", note: "Suhu kurang 2°C" },
      { id: "CP4", label: "Serah Terima", score: 22, status: "pass" },
    ],
    penalties: [
      { label: "Keterlambatan 15 menit", deduction: 5 },
    ],
    payout: 2350000
  };
        ...
        <section className="flex flex-col items-center justify-center py-8 space-y-2">
          <div className="relative h-40 w-40 flex items-center justify-center">
            <svg className="h-full w-full -rotate-90">
              ...
              <circle cx="80" cy="80" r="70" fill="none" stroke="#16a34a" strokeWidth="12" strokeDasharray="440" strokeDashoffset={440 - (440 * scoreData.total) / 100} strokeLinecap="round" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-5xl font-black text-slate-900">{scoreData.total}</span>
              ...
              <Card key={cp.id} className="border-none shadow-sm">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-900">{cp.id}: {cp.label}</p>
                      {cp.note && <p className="text-[10px] text-amber-600 font-medium">{cp.note}</p>}
                    </div>
                  </div>
        ...
        <section className="space-y-3">
          ...
          <div className="p-4 bg-red-50 rounded-2xl border border-red-100 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-bold text-red-900">{scoreData.penalties[0].label}</p>
            </div>
            <span className="font-black text-red-600">-{scoreData.penalties[0].deduction}</span>
          </div>
        </section>
        ...
        <Card className="bg-slate-900 text-white border-none shadow-xl">
          <CardContent className="p-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Estimasi Dana Cair</p>
                <h3 className="text-2xl font-black mt-1">Rp {scoreData.payout.toLocaleString()}</h3>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              <p className="text-[10px] text-slate-400">Dana akan ditransfer otomatis ke wallet vendor setelah verifikasi akhir jam 18:00 WIB.</p>
            </div>
          </CardContent>
        </Card>
