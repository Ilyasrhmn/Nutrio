...
export default function PortalAIAssistantPage() {
  const [messages, setMessages] = useState<{ role: string; content: string; sources: string[] }[]>([
    {
      role: "assistant",
      content:
        "Halo! Saya Asisten MBG. Saya memegang seluruh data Juknis & SOP resmi. Silakan tanya apa saja, atau unggah foto porsi makan/dokumen untuk saya analisa kepatuhannya.",
      sources: [],
    },
  ]);
  ...
  const handleSendMessage = () => {
    ...
    setMessages([
      ...messages,
      { role: "user", content: inputValue, sources: [] },
    ]);
    setInputValue("");
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Berdasarkan Juknis BGN No. 2/2024 hal 14, pendaftaran vendor memerlukan NIB aktif dan Sertifikat SLHS. Untuk porsi makan, minimal protein adalah 15g per sajian.",
                      ...
                      <div className="flex flex-wrap gap-2">
                        {msg.sources.map((src, sIdx) => (
                          <Badge
                            key={sIdx}
                            variant="secondary"
                            className="text-[10px] flex items-center gap-1 cursor-pointer bg-indigo-50 text-indigo-700 border-indigo-100 hover:bg-indigo-100 transition-colors"
                          >
        ...
        <div className="p-6 border-t bg-white">
          <div className="max-w-4xl mx-auto flex items-end gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all">
            <Button
              variant="ghost"
              size="icon"
              className="shrink-0 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-xl h-10 w-10"
            >
        ...
        <div className="p-6">
          ...
          <div className="space-y-3">
            {juknisDocs.map((doc) => (
              <div
                key={doc.id}
                className="p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md hover:border-indigo-200 cursor-pointer transition-all group"
              >
                <div className="flex justify-between items-start mb-2">
                  <span className="text-sm font-bold text-slate-700 truncate pr-2 group-hover:text-indigo-600 transition-colors">
                    {doc.title}
                  </span>
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] font-bold py-0 h-5 border-slate-200"
                >
                  {doc.category}
                </Badge>
              </div>
          ...
          <Card className="mt-8 bg-indigo-900 text-white border-0 shadow-xl overflow-hidden relative">
            ...
            <CardContent className="p-6 relative z-10">
              ...
              <p className="text-[11px] text-indigo-100 leading-relaxed mb-4">
                Sistem AI dapat menganalisa foto porsi makan atau dokumen
                sertifikat secara otomatis.
              </p>
              <Button
                size="sm"
                className="w-full bg-white text-indigo-600 hover:bg-indigo-50 font-bold border-0 h-8 text-[11px]"
              >
                Mulai Upload
              </Button>
            </CardContent>
