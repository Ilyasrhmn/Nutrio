...
export default function VendorOrdersPage() {
  ...
  const load = React.useCallback(async () => {
    try {
      setLoading(true)
      setLoadError(null)
      const data = await ordersService.listVendor()
      setOrders(data)
    } catch (error) {
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-5xl mx-auto min-h-screen">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
          <FileText className="size-6 text-primary" /> Purchase Order Saya
        </h1>
        <p className="text-sm text-slate-500 mt-1">Riwayat pesanan ke supplier.</p>
      </div>
      ...
      <QueryState
        ...
        emptyMessage="Buat pesanan pertama dari halaman Marketplace."
      >
