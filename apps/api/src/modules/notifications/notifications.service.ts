...
export class NotificationsService {
  ...
  async send(opts: SendOptions): Promise<Notification> {
    ...
    try {
      switch (opts.channel) {
        ...
        case "sms":
          this.logger.log(
            `[notif:stub] ${opts.channel} → userId=${opts.userId} | ${opts.body}`,
          );
          break;
      }
      ...
      this.logger.error(
        `[notif] Failed channel=${opts.channel} userId=${opts.userId}: ${message}`,
      );
    }
  ...
  private async sendEmail(opts: SendOptions, notifId: string): Promise<void> {
    if (!this.emailApiKey) {
      this.logger.warn(
        `[notif:email] EMAIL_API_KEY not set — skipping email to userId=${opts.userId}`,
      );
      return;
    }
