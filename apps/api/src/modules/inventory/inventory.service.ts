...
export class InventoryService {
  ...
  async current(userId: string) {
    ...
    return rows.map((row: any) => ({ productId: row.product_id, name: row.name, unit: row.unit, quantity: Number(row.quantity) }));
  }
  ...
  async opname(userId: string, dto: CreateOpnameDto, key?: string) {
    return this.idempotency.execute(userId, key, dto, async (manager, correlationId) => {
      ...
      if (delta === 0) throw new UnprocessableEntityException('Jumlah opname sama dengan saldo stok');
      ...
      const eventId = await this.audit.record(manager, { actorUserId: userId, aggregateType: 'inventory', aggregateId: sourceId, action: 'inventory.opname', after: { productId: dto.productId, unit: dto.unit, countedQuantity: dto.countedQuantity, reason: dto.reason }, correlationId });
      return { body: { productId: dto.productId, unit: dto.unit, quantity: dto.countedQuantity }, eventId };
    });
  }
}
